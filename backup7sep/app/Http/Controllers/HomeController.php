...
class HomeController extends Controller
{
 ...
 public function index()
   ...
   //notification popup
    $modalnotification = DB::table('notifications') ->select('*')->where('status',1)->where('type','!=','message')->take(3)->orderBy('id', 'DESC')->get();
    $count = DB::table('notifications') ->select('*')->where('type','!=','message')->where('deactivate','!=',1)->count();
   ...
   return view('home',compact('newreff','notification','dailycarelogs','notes','schedule'));
   }
